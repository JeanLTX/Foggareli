...
const PIZZAS_SALGADAS = [
    ...
    {
        id: 49, name: "Vegetariana", category: "Pizzas Tradicionais",
        desc: "Molho de tomate, muçarela, palmito, champignon, tomate picado, manjericão fresco e orégano.",
        img: "assets/vegetariana.webp",
        type: "pizza", prices: { P: 59.90, M: 79.90, G: 97.90 }
    },
    {
...
const COMBOS = [
    ...
    },
    {
        ...
        itens: ["Pizza 30cm · 6 fatias", "Refrigerante 1L"],
        price: 75.90,
        obs: "Válido para pizzas tradicionais, exceto Carne Seca e Atum.",
...
let comboStep = 1; // 1: Detalhes, 2: Sabores Salgados, 3: Doces/Bebida, 4: Borda
let comboSelections = {
...
function renderComboStep() {
    ...
    if (comboStep === 1) {
                ...
                <div class="rounded-3xl border-2 border-orange-100 bg-orange-50 p-5 sm:p-7 shadow-sm">
                    ...
                    <div class="space-y-3">
                        ${BORDAS.map(border => {
                            ...
                            const priceLabel = border.id === 'sem_borda'
                                ? 'Incluso no combo'
                            ...
                            return `
                                <button type="button" onclick="selectComboBorder('${border.id}')"
                                    ...
                                    <span class="flex min-w-0 flex-col">
                                        <span class="text-sm font-black text-slate-800">${border.id === 'sem_borda' ? 'Sem borda (Incluso)' : `Borda de ${border.name}`}</span>
                                        <span class="mt-1 text-xs font-bold ${border.id === 'sem_borda' ? 'text-slate-400' : 'text-orange-600'}">${priceLabel}</span>
                                    </span>
        ...
        // ETAPAS DE SELEÇÃO DE SABOR E BEBIDA
        backBtn.classList.remove('hidden');
        summaryContainer.classList.remove('hidden');
        ...
        let isFlavorStep = comboStep === 2 || isPerfeitoStep2;
        ...
        if (isPerfeitoStep2) {
            ...
            nextText.textContent = currentCombo.id === 'individual' ? "Adicionar" : "Próximo";
            nextIcon.setAttribute('data-lucide', currentCombo.id === 'individual' ? 'shopping-cart' : 'arrow-right');
        } else {
            gridTitle = "Escolha o refrigerante";
        ...
        if ((isCasal || isFamilia) && comboStep === 2 || isPerfeitoStep3) {
        ...
        content.innerHTML = `
            <div class="animate-in fade-in slide-in-from-right-4 duration-300" style="padding-top: 1.5rem; padding-bottom: 1.5rem;">
                ...
                ${isFlavorStep ? `<div class="combo-padding grid grid-cols-2 gap-4">${flavorsHtml}</div>` : ''}
                ${drinkHtml}
            </div>
        `;
        updateComboSummary();
...
function nextComboStep() {
    if (comboStep === 1) {
        ...
        if ((currentCombo.id === 'casal' || currentCombo.id === 'familia' || currentCombo.id === 'perfeito') && !comboSelections.drink) {
            return alert('Escolha o refrigerante!');
        }
        comboStep = 4;
        renderComboStep();
    } else {
...
function updateComboSummary() {
    ...
    const needsDrink = (['casal', 'familia'].includes(currentCombo.id) && comboStep === 2)
        || (currentCombo.id === 'perfeito' && comboStep === 3);
    const hasDrink = !!comboSelections.drink;

    if (!hasFlavor) {
        text += "Escolha o sabor...";
    } else if (currentCombo.id === 'perfeito' && comboStep === 3 && !hasSweet) {
        text += "Escolha a pizza doce...";
    } else if ((comboStep === 2 || comboStep === 3) && needsDrink && !hasDrink) {
        text += "Escolha o refrigerante...";
    } else {
        const p1 = ALL_PIZZA_FLAVORS.find(f => f.id === comboSelections.flavors[0]);
        text += p1 ? p1.name : "Pedido";
        if (comboSelections.flavors.length > 1) {
            const p2 = ALL_PIZZA_FLAVORS.find(f => f.id === comboSelections.flavors[1]);
            text += ` / ${p2.name}`;
        }
        if (hasSweet) {
            const sweet = ALL_PIZZA_FLAVORS.find(f => f.id === comboSelections.sweetFlavor);
            text += ` | Doce: ${sweet.name}`;
        }
        if (hasDrink) {
            text += ` | ${comboSelections.drink === 'fanta' ? 'Fanta' : 'Guaraná'}`;
        }
        ready = true;
    }
